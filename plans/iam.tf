data "aws_iam_policy_document" "feed_processor_binarydefense_assume_role_policy" {
  statement {
...
data "aws_iam_policy_document" "feed_processor_binarydefense_iam_policy" {
  statement {
  ...
  statement {
    ...
    resources = [
      "arn:aws:dynamodb:${local.aws_default_region}:${local.aws_master_account_id}:table/${lower(var.app_env)}_ews_binarydefense",
    ]
  }
}
resource "aws_iam_role" "feed_processor_binarydefense_role" {
  name               = "${lower(var.app_env)}_feed_processor_binarydefense_lambda_role"
  assume_role_policy = data.aws_iam_policy_document.feed_processor_binarydefense_assume_role_policy.json
  lifecycle {
    create_before_destroy = true
  }
}
resource "aws_iam_policy" "feed_processor_binarydefense_policy" {
  name   = "${lower(var.app_env)}_feed_processor_binarydefense_lambda_policy"
  path   = "/"
  policy = data.aws_iam_policy_document.feed_processor_binarydefense_iam_policy.json
}
resource "aws_iam_role_policy_attachment" "policy_attach" {
  role       = aws_iam_role.feed_processor_binarydefense_role.name
  policy_arn = aws_iam_policy.feed_processor_binarydefense_policy.arn
}
