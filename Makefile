...
output: env
	@echo -e $(bold)$(primary)feed_processor_binarydefense_arn$(clear) = $(shell terraform -chdir=plans output feed_processor_binarydefense_arn)
	@echo -e $(bold)$(primary)feed_processor_binarydefense_role$(clear) = $(shell terraform -chdir=plans output feed_processor_binarydefense_role)
	@echo -e $(bold)$(primary)feed_processor_binarydefense_role_arn$(clear) = $(shell terraform -chdir=plans output feed_processor_binarydefense_role_arn)
	@echo -e $(bold)$(primary)feed_processor_binarydefense_policy_arn$(clear) = $(shell terraform -chdir=plans output feed_processor_binarydefense_policy_arn)
...
tail-aws-logs: ## Install using pipx install awscliv2
	awsv2 logs tail "/aws/lambda/$(shell sed -e 's/\(.*\)/\L\1/' <<< "${APP_ENV}")-feed-processor-binarydefense" --follow
